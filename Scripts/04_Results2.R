...
# lm
summary(lm(formula = ibp ~ (proportion_robinier*classe_age*bois_mort), data = RobDataset))
# polynomial
...
# ANOVA
# (Type II tests)
Anova(aov(formula = ibp ~ proportion_robinier*classe_age, data = RobDataset))

#IBP regression
matrix_coef <- summary(lm(ibp ~ splines::bs(proportion_robinier, df = 3), data = RobDataset))$coefficients # Extract coefficients in matrix
...
ggplot(data = RobDataset) +
    ...
    # stat_smooth(method = lm, formula = y ~ splines::bs(x, df = 3), col = "red") +
    labs(
        # x = "Abondance robinier",
         # y = "Pourcentage (%)",
         title = "IBP",
         subtitle = "Adj R2 = 0.1168 ; P = 0.0001"     
    )
test <- (RobDataset$proportion_robinier*RobDataset$classe_age)
...
par(mfrow = c(2, 2))
plot(lm(formula = ibp ~ (proportion_robinier*classe_age), data = RobDataset))
# ibp oui 
